...
func LocationCreate(c *fiber.Ctx) error {	
	...
	if locationQR == "" {return utils.Error(c, 400, messages.LocationQRRequired)} 
	if locationName == "" {return utils.Error(c, 400, messages.LocationNameRequired)} 
...
func LocationSetLocation(c *fiber.Ctx) error{
	...
	if locationUID == setLocationUID{return utils.Error(c, 400, messages.LocationSelfError)}
